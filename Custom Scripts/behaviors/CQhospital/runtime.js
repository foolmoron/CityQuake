...
//           vvvvvvvvvv
cr.behaviors.CQhospital = function(runtime)
{
...
(function ()
{
	// *** CHANGE THE BEHAVIOR ID HERE *** - must match the "id" property in edittime.js
	//                               vvvvvvvvvv
	var behaviorProto = cr.behaviors.CQhospital.prototype;
			...
			for (var i = 0; i < this.SURROUNDING_TILES.length; i++){
				...
				if (this.tileX + xOffset >= 0 && this.tileX + xOffset < CQ.GRID_SIZE &&
				  this.tileY + yOffset >= 0 && this.tileY + yOffset < CQ.GRID_SIZE){
					var houseBehavior = CQ.hasBehavior(CQ.objGrid[this.tileX + xOffset][this.tileY + yOffset], "CQHouse");
					if (houseBehavior){
